...
pub fn build_defalut_pipeline() -> Rc<RefCell<Pipeline>> {
    ...
    pipeline.borrow_mut().insert(Pass::new(
        get_default_pipeline_default_pass_name(),
        Default::default(),
    ));
    pipeline
}

pub fn get_default_pipeline_default_pass_name() -> &'static str {
    "main"
}

pub fn build_default_pipeline_material(shader: Rc<RefCell<Shader>>) -> Rc<RefCell<MaterialGroup>> {
