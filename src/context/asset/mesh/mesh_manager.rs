...
impl MeshManager {
    ...
    pub fn create_from_position_normal(
        &mut self,
        indices: &Vec<usize>,
        positions: &Vec<f32>,
        normals: &Vec<f32>,
    ) -> Result<MeshHandle, MeshError> {
        let mesh = Mesh::from_position_normal(indices, positions, normals);
        let mesh_gpu_wrapper = MeshGPUWrapper::from_mesh(&mesh);
        Ok(self.meshes.insert(mesh_gpu_wrapper))
    }

    pub fn create_from_position_texcoord(
        &mut self,
        indices: &Vec<usize>,
        positions: &Vec<f32>,
        texcoords: &Vec<f32>,
    ) -> Result<MeshHandle, MeshError> {
        let mesh = Mesh::from_position_texcoord(indices, positions, texcoords);
        let mesh_gpu_wrapper = MeshGPUWrapper::from_mesh(&mesh);
        Ok(self.meshes.insert(mesh_gpu_wrapper))
    }

    pub fn create_from_position_normal_texcoord(
        &mut self,
        indices: &Vec<usize>,
        positions: &Vec<f32>,
        normals: &Vec<f32>,
        ...
        let mesh = Mesh::from_position_normal_texcoord(indices, positions, normals, texcoords);
        let mesh_gpu_wrapper = MeshGPUWrapper::from_mesh(&mesh);
        Ok(self.meshes.insert(mesh_gpu_wrapper))
    }
    ...
    pub fn create_from_obj_path(&mut self, path: &str) -> Result<MeshHandle, MeshError> {
     ...
     pub fn create_from_obj_in_bytes(&mut self, data: &[u8]) -> Result<MeshHandle, MeshError> {
        let mesh = Mesh::load_obj_from_memory(data)?;
        let mesh_gpu_wrapper = MeshGPUWrapper::from_mesh(&mesh);
        Ok(self.meshes.insert(mesh_gpu_wrapper))
    }
